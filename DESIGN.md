...
- **`NextSectionCue`** (`src/components/NextSectionCue.tsx`): the touch counterpart to that chain, rendered by `Section` as the last thing in every section's scroll content except the last. A ledger row in the Connect directory's idiom — `MetaLabel` in the margin, the destination in display serif, a chevron at the edge — because the system already has one way of saying "this row takes you somewhere", and the one genuinely new control on the site should not be a new invention. Its chevron points **down**, not along the camera's real travel vector: the canvas sends you up to About and right to Projects, but the control means "keep reading", and an arrow describing the camera would be answering a question nobody asked.
...
### Named Rules
**The Wheel-Is-Not-Navigation Rule.** The scroll chain is driven by `wheel`, which a touchscreen never fires — so for as long as it was the only way between sections, a phone had no scroll navigation at all and the drawer menu was the sole exit from a section. Any affordance the chain provides on a desktop needs a touch counterpart that does not depend on a gesture the device cannot produce; `NextSectionCue` is that counterpart, gated on `hover: none` rather than a width breakpoint, because the distinction that matters is input, not screen size. A narrow desktop window still has a wheel; an iPad at 1024px does not.

**The Shell-Is-Measured-In-dvh Rule.** Every full-height element derives from `--app-height` / `--viewport-height` (`src/index.css`), never a bare `100vh`. On iOS Safari `100vh` is the *large* viewport — the height with the toolbar retracted — and a normal page earns that height by scrolling, which retracts the toolbar. This one never can: `html` and `body` are `overflow: hidden` and all scrolling happens inside the section containers, so the toolbar stays expanded for the whole visit and the bottom ~50px of every full-height element sits permanently beneath it. That is not a cosmetic crop — it shipped as a bug where the mobile drawer's theme toggle, pinned to `bottom-0`, could not be tapped at all on an iPhone. Because the body genuinely cannot scroll, `dvh` is also stable here: there is no toolbar-retraction reflow to jitter against.
